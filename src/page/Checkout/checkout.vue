        ...
        <div slot="content">
          <ul class="address-item-list clearfix">
            <li v-for="(item,i) in addList"
                ...
                @click="chooseAddress(item.id, item.signer, item.phone, item.address)">
           <span v-if="addressId === item.id" class="pa">
             ...
             <path
              ...
              <p>收货人: {{item.signer}} {{item.is_default ? '(默认地址)' : ''}}</p>
                ...
                <div class="cart-group divide pr" :data-productid="item.id">
                  <div class="cart-top-items">
                    <div class="cart-items clearfix">
                      ...
                      <div>
                        <!--总价格-->
                        <div class="subtotal" style="font-size: 14px">¥ {{item.goods.price * item.nums}}</div>
                        <!--数量-->
      ...
      <y-popup :open="popupOpen" @close='popupOpen=false' :title="popupTitle">
        <div slot="content" class="md" :data-id="msg.addressId">
          ...
          <y-button text='保存'
                    ...
                    @btnClick="save({id:msg.addressId,signer:msg.userName,phone:msg.tel,address:msg.streetName})">
          </y-button>
        </div>
      </y-popup>
    </div>
...
<script>
  import { getCartList, addressList, addressUpdate, addressAdd, addressDel, submitOrder, productDet } from '/api/goods'
  ...
  export default {
    ...
    methods: {
      ...
      _addressList () {
        addressList().then(res => {
          if (res.length) {
            ...
            this.userName = res[0].signer
            this.tel = res[0].phone
            this.streetName = res[0].adress
          } else {
            this.addList = []
          }
        })
      ...
      _addressUpdate (params) {
        addressUpdate(params.id, params).then(res => {
          this._addressList()
        }).catch(err => {
          for (var index in err) {
            this.message(err[index][0])
          }
        })
      },
      _addressAdd (params) {
      ...
      _productDet (productId) {
        productDet(productId).then(res => {
          ...
          // item.checked = true
          // item.productImg = item.goods_front_image
