    ...
    <y-shelf v-bind:title="orderTitle">
      <div slot="content">
        <div v-loading="loading" element-loading-text="加载中..." style="min-height: 10vw;" v-if="orderList.length">
          <div class="orderStatus" v-if="orderStatus !== 4 && orderStatus !== 5">
            <el-steps :space="390" :active="orderStatus">
          ...
          <div class="orderStatus-close" v-if="orderStatus === 4">
            <el-steps :space="780" :active="2">
              <el-step title="下单" v-bind:description="createTime"></el-step>
              <el-step title="交易取消" v-bind:description="closeTime"></el-step>
            </el-steps>
          </div>
          <div class="orderStatus-close" v-if="orderStatus === 5">
            <el-steps :space="780" :active="2">
              <el-step title="下单" v-bind:description="createTime"></el-step>
              <el-step title="交易失效" v-bind:description="closeTime"></el-step>
            </el-steps>
          </div>
          <div class="status-now" v-if="orderStatus === 1">
            <ul>
              ...
              <li class="button">
                ...
                <el-button @click="_cancelOrder(orderSn)" size="small">取消订单</el-button>
              </li>
            </ul>
            <p class="realtime">
              <span>您的付款时间还有 </span>
          ...
          <div class="status-now" v-if="orderStatus === 5">
            <ul>
              <li class="status-title"><h3>订单状态：已失效</h3></li>
            </ul>
            <p class="realtime">
              <span>您的订单已关闭。</span>
            </p>
          ...
          <div class="goods-table">
            <div class="cart-items" v-for="(item,i) in orderList" :key="i">
              <a @click="goodsDetails(item.goods.id)" class="img-box"><img :src="item.goods.goods_front_image" alt=""></a>
              <div class="name-cell ellipsis">
                <a @click="goodsDetails(item.goods.id)" title="" target="_blank">{{item.goods.name}}</a>
              </div>
              <div class="n-b">
                <div class="price">¥ {{parseFloat(Number(item.goods.price).toFixed(2))}}</div>
                <div class="goods-num">{{item.nums}}</div>
                <div class="subtotal"> ¥ {{parseFloat(Number(item.goods.price * item.nums).toFixed(2))}}</div>
              </div>
            </div>
          </div>
          <!--合计-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      _getOrderDet () {
        getOrderDet(this.orderSn).then(res => {
          this.orderStatus = res.pay_status
          this.orderList = res.order_goods
          ...
          this.createTime = res.ctime
          if (res.pay_status === 1) {
            this.countTime = this.orderSn.substr(0, 13) * 1 + 60 * 60 * 1000
          } else {
            this.payTime = res.pay_time
          }
          if (res.pay_status === 3) {
            this.finishTime = res.end_time
          } else if (res.pay_status === 4 || res.pay_status === 5) {
            this.closeTime = res.end_time
          }
          this.loading = false
        })
      },
      _cancelOrder () {
        updateOrder(this.orderSn, {pay_status: 4}).then(res => {
          this._getOrderDet()
        }).catch(_ => {
          this.message('取消失败')
        })
