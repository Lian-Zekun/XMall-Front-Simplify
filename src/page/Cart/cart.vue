                ...
                <div class="cart-group divide pr" :data-productid="item.id">
                  <div class="cart-top-items">
                    <div class="cart-items clearfix">
                      ...
                      <div>
                        <!--总价格-->
                        <div class="subtotal" style="font-size: 14px">¥ {{parseFloat(Number(item.goods.price * item.nums).toFixed(2))}}</div>
                        <!--数量-->
                        ...
                        <!--价格-->
                        <div class="price1">¥ {{parseFloat(Number(item.goods.price).toFixed(2))}}</div>
                      </div>
                    </div>
                  </div>
                </div>
  ...
  export default {
    ...
    methods: {
      ...
      _cartEdit (productId, productNum, checked) {
        cartEdit(productId, {nums: productNum, checked: checked}).then(res => {
          this.EDIT_CART(
            {
              productId: res.id,
              productNum: res.nums,
      ...
      delChecked () {
        getCartList().then(res => {
          res.forEach(item => {
            if (item.checked) {
              this.EDIT_CART({productId: item.id})
            }
          })
        })
        delCartChecked().catch(_ => {
          this.message('删除失败')
        })
      }
    },
